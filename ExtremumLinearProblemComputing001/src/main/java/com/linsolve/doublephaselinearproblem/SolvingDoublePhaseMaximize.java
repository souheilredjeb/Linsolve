...
public class SolvingDoublePhaseMaximize 
{
	private  SolvingAuxiliaryProblemDoublePhase solver_phase_I;
	...
	public SolvingDoublePhaseMaximize() 
	{

	}
	
	public SolvingDoublePhaseMaximize(SolvingAuxiliaryProblemDoublePhase solver_phase_I) 
	{
	...
	public Integer compute_pivot_column() throws Exception
	{
		...
		if(this.optimality.equals(Boolean.FALSE))
		{
			for(int j=0;j<this.zj_cj_phase_II.length;j++)
			{	
				if(this.zj_cj_phase_II[j].compareTo(maximum_value)>0)
				{
					pivot_column=j;
					maximum_value=this.zj_cj_phase_II[j];
				}
			}
	...
	public  void build_b_comporator_vector()
	{
		...
		{
			if(augmented_matrix_phase_II[i][augmented_matrix_phase_II[0].length-1]>=0 && this.pivot_column !=null && augmented_matrix_phase_II[i][pivot_column]>0)
			{
	...
	public  void reevaluate_zj_cj()
	{
		if(this.pivot_column !=null && this.pivot_line !=null)
		{
			 for(int j=0;j<this.getAugmented_matrix_phase_II()[0].length-1;j++)
			 {
				zj_cj_phase_II[j]=0.0;			
			 }
			 for(int j=0;j< this.getAugmented_matrix_phase_II()[0].length-1;j++) 
	...
	public Double compute_goal_function() throws Exception
	{
		...
		for(int i=0;i<this.getB_vector().length;i++)
		{
			if(this.base[i]<this.c_vector.length)
			{
				z=z+this.getSolver_phase_I().getBam().getC_vector()[base[i]]*b_augmented_matrix[i];
			}
		}
	...
	public static void main(String[] args) throws Exception 
	{
		...
		try
		{
			SolvingLinearProblemMain slpm = new SolvingLinearProblemMain(input6);
			HashMap<Integer, Double> solution_computed=(HashMap<Integer, Double>) slpm.getSdpmax().enumerate_based_variables_values();
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public SolvingAuxiliaryProblemDoublePhase getSolver_phase_I() {
		return solver_phase_I;
	}

	public void setSolver_phase_I(SolvingAuxiliaryProblemDoublePhase solver_phase_I) {
		this.solver_phase_I = solver_phase_I;
	}
